(function(decaf, $, _, ace, window) {
  ...
  function REPL () {
    ...
    this.input = new Editor('.decaf-repl-input .ace_editor', 'ace/mode/coffee').editor;
    this.input.setValue(UriUtils.decode(state.code || ''));

    this.output = new Editor('.decaf-repl-output .ace_editor', 'ace/mode/javascript').editor;
    this.output.setReadOnly(true);
    this.output.setHighlightActiveLine(false);
    this.output.setHighlightGutterLine(false);

    this.$errorReporter = $('.decaf-repl-errors');
    this.$consoleReporter = $('.decaf-repl-console');
    this.$toolBar = $('.decaf-repl-toolbar');
  }
  ...
  REPL.prototype.compile = function () {
    ...
    try {
      transformed = decaf.compile(code);
    } catch (err) {
